...
All 100 packages grouped by domain.

| Domain | Packages |
|--------|----------|
| **transcriptomics** (33) | `affy`, `apeglm`, `AUCell`, `batchelor`, `dada2`, `DESeq2`, `DropletUtils`, `edgeR`, `EnhancedVolcano`, `fgsea`, `GEOquery`, `glmGamPoi`, `GSEABase`, `GSVA`, `illuminaio`, `limma`, `MAST`, `MetaboCoreUtils`, `metapod`, `monocle`, `msa`, `pathview`, `scater`, `scDblFinder`, `scran`, `scuttle`, `siggenes`, `SingleR`, `SpatialExperiment`, `sva`, `TrajectoryUtils`, `treeio`, `tximport` |
| **genomics** (23) | `annotate`, `biocViews`, `Biostrings`, `BSgenome`, `clusterProfiler`, `ComplexHeatmap`, `ConsensusClusterPlus`, `DECIPHER`, `DNAcopy`, `GenomicAlignments`, `GenomicFeatures`, `GenomicRanges`, `ggbio`, `ggtree`, `GOSemSim`, `Gviz`, `KEGGREST`, `OrganismDbi`, `Rsamtools`, `rtracklayer`, `ShortRead`, `TFBSTools`, `txdbmaker` |
| **general** (16) | `basilisk`, `Biobase`, `biovizBase`, `DOSE`, `ExperimentHub`, `gdsfmt`, `genefilter`, `geneplotter`, `graphite`, `gypsum`, `MultiAssayExperiment`, `pcaMethods`, `qvalue`, `ResidualMatrix`, `seqlogo`, `topGO` |
| **proteomics** (8) | `mixOmics`, `MsCoreUtils`, `MSnbase`, `mzID`, `mzR`, `PSMatch`, `QFeatures`, `STRINGdb` |
| **variant-calling** (4) | `AnnotationHub`, `snpStats`, `VariantAnnotation`, `vsn` |
| **metagenomics** (4) | `biomformat`, `DirichletMultinomial`, `microbiome`, `phyloseq` |
| **single-cell** (3) | `SingleCellExperiment`, `TreeSummarizedExperiment`, `zellkonverter` |
| **epigenomics** (3) | `bumphunter`, `ChIPseeker`, `minfi` |
| **enrichment** (2) | `enrichplot`, `ReactomePA` |
| **annotation** (2) | `biomaRt`, `KEGGgraph` |
| **imaging** (2) | `EBImage`, `flowCore` |
